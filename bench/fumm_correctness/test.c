...
float C_kfumm [BENCH_M * BENCH_N2];

float A_buffer [BENCH_K * BENCH_M];
float B1_buffer [NC * BENCH_K];
float B2_buffer [NC * BENCH_N2];
...
int main() {
    ...
    skfumm_neon_4x16(BENCH_M, BENCH_N1, BENCH_N2, BENCH_K, 1.0, 1.0, A, 1, BENCH_K, A_buffer, B1, 1, BENCH_N1, B1_buffer, B2, 1, BENCH_N2, B2_buffer, 1.0, C_kfumm, 1, BENCH_N2, C_buffer);
    
    printf("same? %d\n", matrix_comp(C1, C2_2, BENCH_N2, BENCH_M));
    printf("same? %d\n", matrix_comp(C_kfumm, C2_2, BENCH_N2, BENCH_M));
